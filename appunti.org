...
*** Parte 1
    ...
    Si risolve semplicemente implementando le regole descritte per il
    lancio dei dadi.

*** Parte 2

    Qui sorgono i problemi: dobbiamo determinare in quanti universi
    vince il player 1 e in quanti vince il player 2.

    Il numero degli universi in cui vincono i due giocatori sono
    determinati dalle loro posizioni e dal loro punteggio corrente.

    Quindi creiamo una funzione search(p1, p2, s1, s2) che restituisce
    i due valori w1 e w2 cioè il numero di universi in cui vince il
    giocatore 1 e il giocatore 2.

    Il giocatore p1 sarà quello che effettuerà il lancio dei dadi.

    Lanciando i dadi tre volte avremmo i seguenti punti possibili:

    #+BEGIN_SRC python
      [3, 4, 5, 4, 5, 6, 5, 6, 7, 4, 5, 6, 5, 6, 7, 6, 7, 8, 5, 6, 7, 6, 7, 8, 7, 8, 9]
    #+END_SRC

    Per ogni possibile risultato dei dadi:

    1) se il punteggio del giocatore p1 ns1 >= 21 aggiungeremo 1 alle
       vittorie di p1 (w1 += 1).
    2) altimenti invocheremo search(p2, np1, s2, ns1) e aggiungeremo
       le vittorie di p2 e di p1 alle vittorie correnti. Notare che
       essendo il turno di p2, p2 è il primo argomento passato alla
       funzione.

    Quindi la funzione è ricorsiva ed essendo i valori di p1, p2, s1,
    s2 limitati, viene richiamata con gli stessi argomenti molte
    volte.

    Per evitare di calcolare nuovamente valori già determinati si usa
    la tecnica di programmazione dinamica detta memoization: una volta
    effettuato un calcolo oneroso memorizziamo il risultato in un
    dizionario.

    Quando la funzione sarà richiamata con gli stessi argomenti,
    anziché effettuare il calcolo, sarà restituito il valore
    memorizzato in precedenza.

    In python il decorator @cache (from functools import cache)
    effettua questo servizio automaticamente.

    In C o in C++ sarà necessario memorizzare i valori in una matrice.

** Day 22

*** Parte 1

    La prima parte può essere risolta facilmente usando una sparse
    map, cioè un dizionario in cui teniamo traccia dei cubi accesi.

*** Parte 2

    Purtroppo non possiamo usare l'approccio precedente perché le
    dimensioni dei cubi sono enormi.

    Possiamo però trattare i cuboidi come entità geometriche, cioè
    come coppie di punti p0, p1 che li delimitano.

    Per questa rappresentazione dobbiamo sviluppare le due operazioni
    di somma e differenza.

    Per facilitare l'analisi consideriamo i cuboidi come entità in due
    dimensioni (cioè dei rettangoli) e il loro volume pari alla loro
    area.

    Dati due cuboidi A e B, la somma A+B si articola in due casi:

    1) se A e B non si intersecano il volume del risultato
       dell'operazione sarà la somma dei volumi di A e B.
    2) se A e B si intersecano il volume del risultato dell'operazione
       sarà la somma dei volumi di A e B meno il volume
       dell'intersezione tra A e B. Se non sottraiamo l'intersezione
       sarà conteggiata due volte, una per A e una per B.

    Si intravede che il risultato di ogni somma saranno due liste di
    cuboidi: la lista dei cuboidi "on" e la lista delle intersezioni
    "off".

    Dati due cuboidi A e B, la differenza A-B si articola in due casi:

    1) se A e B non si intersecano il volume del risultato
       dell'operazione sarà uguale al volume di A.
    2) se A e B si intersecano il volume al termine dell'operazione
       sarà uguale al volume di A meno il volume dell'intersezione tra
       A e B.

    Anche in questo caso il risultato dovrà essere memorizzato in due
    liste di cuboidi.

    Quindi creiamo due liste: la lista *ON* che avrà tutti i cuboidi
    accesi e la lista *OFF* con tutte le intersezioni ridondanti da
    sottrarre.

    Il volume dei cuboidi accesi in ogni momento sarà dato dalla somma
    dei volumi di *ON* meno la somma dei volumi di *OFF*.

    Nel caso dell'operazione "on" dovremo calcolare le intersezioni
    tra il cuboide specificato nell'istruzione e ogni cuboide della
    lista *ON*. Queste intersezioni saranno inserite nella lista
    *OFF*.

    Dobbiamo inoltre calcolare ogni intersezione tra cuboide
    dell'istruzione e ogni elemento della lista *OFF* e inserire
    queste intersezioni nella lista *ON*.

    Per capirne il perché pensiamo a due rettangoli A, B che si
    intersecano. La loro somma produce le seguenti liste:
    ...
    *OFF* = [A︀∩B]
    ...
    Aggiungiamo ora un rettangolo C che ricopre entrambi A e B:
    ...
    *OFF* = [A∩B, A∩C, B∩C]
    ...
    L'area totale deve essere pari all'area di C:

    #+BEGIN_EXAMPLE
    X = Area(A) + Area(B) + Area(C) - Area(A∩B) - Area(A∩C) -
    Area(B∩C)
    #+END_EXAMPLE

    Poiché C compre interamente sia A che B, A∩C = A e B∩C = B:

    #+BEGIN_EXAMPLE
    X = Area(A) + Area(B) + Area(C) - Area(A∩B) - Area(A) - Area(C)

    X = Area(C) - Area(A∩B)
    #+END_EXAMPLE

    Il risultato ottenuto non è corretto. Per farlo tornare dobbiamo
    aggiungere il rettangolo A∩B∩C alla lista *ON*, che si ottiene
    dall'intersezione di C con gli elementi iniziali della lista
    *OFF*:
    ...
    *OFF* = [A∩B, A∩C, B∩C]
    ...
    #+BEGIN_EXAMPLE
    X = Area(A) + Area(B) + Area(C) + Area(A∩B∩C) - Area(A∩B) - Area(A∩C) -
    Area(B∩C)

    X = Area(A) + Area(B) + Area(C) + Area(A∩B) - Area(A∩B) - Area(A) - Area(B)

    X = Area(C)
    #+END_EXAMPLE

    L'operazione "off" è praticamente identica, tranne per l'ultimo
    passaggio: il cuboide dell'istruzione non deve essere inserito in
    nessuna lista.

    Non resta che partire da due liste vuote ed eseguire le istruzioni
    per ottenere la risposta alla domanda: sarà pari alla somma dei
    volumi dei cuboidi *ON* meno la somma dei volumi dei cuboidi
    *OFF*.

    NOTA: nel repository è presente anche una soluzione alternativa
    che effettua l'effettiva divisione geometrica dei cubi e il cui
    tempo di esecuzione è un ordine di grandezza inferiore in tutte le
    implementazioni.
...
*** Parte 1
    ...
    Dobbiamo effettuare una ricerca in un grafo delle decisioni: ogni
    decisione è uno spostamento di un anfipode sulla mappa tra tutti
    quelli possibili.

    Il risultato di ogni decisione è una nuova mappa con associata
    l'energia necessaria per spostare l'anfipode più l'energia
    necessaria per raggiungere la mappa precedente.

    Una ricerca efficiente si ottiene con l'algoritmo di Dijkstra:
    ...
    La funzione ausiliaria decisions(m) elenca le possibili nuove
    mappe ottenute dallo spostamento di un anfipode partendo alla
    mappa m, nonché l'energia richiesta per lo spostamento.

*** Parte 2

    Come la parte 1 ma con una mappa modificata con le due righe
    aggiuntive.

    L'algoritmo risolve questa mappa anche più velocemente della
    precedente.

    L'implementazione Python impiega la bellezza di 5 secondi mentre
    l'implementazione C impiega 0.13 secondi.

    In realtà le implementazioni C e C++ nel repository usano
    l'algorimo di ricerca A* anziché Dijkstra: vengono preferite le
    mappe più vicine alla soluzione in base a una funzione euristica
    che stima la distanza dall'obbiettivo.

    La stima è la somma dell'energia già spesa e dell'energia
    necessaria per portare gli anfipodi dalla loro posizione corrente
    alla coordinata X della loro stanza.

    Non è importante che il risultato sia esatto, basta che non sia
    sovrastimato e in questo caso non lo è: la coordinata y degli
    anfipodi non è nemmeno conteggiata.

    L'uso di A* riduce il tempo di ricerca della metà.

    Ho provato a implementare lo stesso algoritmo in Python ma il
    costo abnorme delle invocazioni di funzione domina tutta la
    ricerca e alla fine l'algorimo A* risulta più lento.

** Day 24

*** Parte 1

    E' inutile programmare una ALU che interpreti il programma se
    prima non lo studiamo, anche perché dovremo fare una ricerca in
    uno spazio di 9**14 numeri, con tempi improponibili.

    Il programma deve analizzare un numero di 14 cifre ma è privo di
    istruzioni di loop e quindi dovranno esserci 14 frammenti di
    codice che caricano la cifra per ciascuna posizione con
    l'istruzione inp e poi la elaborano.

    E infatti guardando il codice è proprio così.

    I frammenti di codice che elaborano ciascuna posizione sembrano
    quasi uguali: cambiano solo dei literals in corrispondenza di
    alcune istruzioni.

    Confrontando ciascun frammento con l'altro possiamo determinare le
    istruzioni che cambiano ed estrarre i tre valori, a, b, c per
    ciascuna delle 14 cifre da elaborare.

    Inoltre, analizzando il codice manualmente, vediamo che ciascun
    frammento è equivalente al seguente codice python:
    ...
    Gli argomenti a, b, c sono i valori estratti per il frammento, d è
    la cifra caricata dall'istruzione `inp w` e z è il valore
    precedente del registro z.

    La funzione ritorna il nuovo valore di z.

    Possiamo costruire un algoritmo di backtracking che faccia una
    ricerca esaustiva di tutte le cifre nelle 14 posizioni, partendo
    dalla prima e determinando le sequenze che generano un valore
    finale z = 0.

    Per ogni posizione partiamo dalla cifra 9 e arriviamo a 1,
    eliminando le cifre che non sono ammissibili in base ai valori di
    a, b e c.

    Infatti se b > 0 tutte le cifre sono possibili, se invece b < 0
    sono possibili solo le cifre d che soddisfano la relazione `z %
    26 + b == d`.
    ...
    La funzione è invocata con i seguenti argomenti: backtrack([], 0,
    0, clist).

    Il parametro clist è la lista dei literals a,b,c per ogni
    frammento di codice.

*** Parte 2

    Per trovare il valore minimo ammissibile possiamo ricercare le
    cifre nell'ordine inverso partendo dalla 1 fino alla 9,
    modificando l'algoritmo.

    Oppure possiamo continuare nella ricerca precedente trovando tutte
    le soluzioni e la risposta sarà l'ultima soluzione ammissibile:
    ...
    Ho scelto la seconda opzione perché le modifiche al codice sono
    minime e il tempo di esecuzione è accettabile (0.8 secondi in
    python, 0.01 secondi in C).

** Day 25

*** Parte 1

    Bisogna aggiornare la mappa seguendo le regole date in due step
    successivi, muovendo prima le oloturie (ROTFL) ">" e poi le
    oloturie "v".

    Non possiamo aggiornare la mappa in-place perché tutte le oloturie
    di un tipo si muovono simultaneamente.

    Quindi dobbiamo salvare il risultato in una nuova mappa in cui
    spostiamo solo le oloturie di un tipo alla volta.

    Infine dobbiamo avere anche la mappa iniziale per confrontare
    punto di partenza e il risultato finale e vedere se le oloturie
    sono tutte bloccate.

    Quindi per effettuare uno step abbiamo bisogno di tre mappe. Al
    termine dello step determiniamo se le oloturie sono bloccate e in
    quel caso ci fermiamo.

*** Parte 2

    Bonus!!! Siamo arrivati alla fine di un'improbabile storia
    subacquea: se ti cadono le chiavi in mare è naturale che finiscano
    nella fossa delle Marianne... </g>

    Ma finalmente Babbo Natale potrà usare la sua slitta, hurrah!

    Buon Natale 2021 e arrivedorci al prossimo Advent of Code :)
