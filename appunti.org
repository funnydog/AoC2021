...
*** Parte 1
    ...
    step. Per segnare un polpo che ha già lampeggiato ne impostiamo
    l'energia a un valore negativo.

    La funzione che incrementa l'energia di ogni polpo è ricorsiva:

    1) controlla se il polpo ha energia negativa (ha già lampeggiato)
    ...
    2) aumenta l'energia del polpo di 1 e se supera il livello 9 fa
       lampeggiare il polpo:

       1) lo segna come già lampeggiato impostando l'energia a -1;

       2) incrementa l'energia dei polpi vicini.

    Dopo aver richiamato la funzione per ogni polpo della mappa alcuni
    polpi avranno energia negativa: sono i polpi che hanno lampeggiato
    durante lo step.
