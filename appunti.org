...
*** Part1
    ...
    L'altezza del foglio diventa fy.
    ...
    La larghezza del foglio diventa fx.
...
*** Parte 2
    ...
    Per stampare il risultato inseriamo i punti della lista in un set
    e per ogni punto (x, y), con 0 <= x < width e 0 <= y < height,
    verifichiamo se il punto è nel set. In quel caso stampiamo in '#'
    altrimenti uno spazio ' '.

    Nella versione C e C++ ordiniamo la lista e la stampiamo dall'alto
    verso il basso.
