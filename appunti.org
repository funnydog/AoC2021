...
*** Parte 2
    ...
    Per lo stesso ragionamento eliminiamo anche "bcdefg" dalla
    cifra 9, perché non ha i segmenti "ab" che sono in comune con la
    cifra 1, e "bcdefg" dalla cifra 0.
    ...
    Ora procediamo con la cifra 3 che ha rappresentazione "abcdf". La
    cifra 9 ha tutti i segmenti in comune con 3. Quindi eliminiamo
    "abcdeg" dai segmenti di 9. La cifra 9 è determinata.
    ...
    Dobbiamo ripetere questo passaggio confrontando i segmenti in
    comune tra le cifre determinate e quelle ancora indeterminate,
    eliminando gli elementi impossibili, fino a quando tutte le cifre
    saranno determinate.

    Per farlo abbiamo bisogno di una matrice che ci dica quanti
    segmenti in comune esistono tra due cifre arbitarie i e j.

    Questa matrice è di facile costruzione ed è simmetrica perché
    l'operatore and è commutativo:

    M[i][j] = bitcount(encode(seg[i]) & encode(seg[j]))

    Con questa matrice è facile eliminare i candidati errati: se i è
    una cifra con una rappresentazione predeterminata e j ha più
