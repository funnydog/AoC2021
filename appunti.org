...
*** Parte 1
    ...
    1) dobbiamo determinare l'estensione della mappa durante
       l'inserimento dei light pixels.

    2) l'algoritmo può trasformare una zona completamente dark in una
       zona completamente light.

    Il punto 1) è facilmente risolvibile mantendo le variabili xmin,
    xmax e ymin e ymax e aggiornandole ogni volta che inseriamo un
    punto nel dizionario.

    Per creare la nuova mappa partiremo dalle coordinate (xmin-2,
    ymin-2) e finiremo alle coordinate (xmax+2, ymax+2).

    Per quanto riguarda il punto 2) se l'algoritmo inizia con un light
    pixel '#' tutte le aree 3x3 di dark pixel si trasformano in light
    pixel perché generano un indice pari a 0.
    ...
    Inizialmente il background è di dark pixel '.' e nella mappa sono
    memorizzati i light pixel '#'.

    Alla prima iterazione il background si trasformerà in light pixel
    ...
    la risposta alla domanda sarebbe infinito. Infatti alla poizione
    511 l'algoritmo avrà sicuramente un dark pixel '.'.

    Il nuovo background diventerà di dark pixel '.' e salveremo nella
    mappa i light pixel '#'.

    Dopo questi due enhance() la mappa conterrà sicuramente i light
